...
client = MongoClient("mongodb+srv://2day4uindia:mrMInKpLaFbULK5Z@cluster0.kifo6xm.mongodb.net/?retryWrites=true&w=majority")
...
api_key = "AIzaSyCrt1weuvjBcnIytIrtPYKjBLRdKSsaoP4"
...
def get_channel_details(channel_id):
    ...
    info['channel_name'] = channel_details['snippet']['title']
    info['playlist_id'] = channel_details['contentDetails']['relatedPlaylists']['uploads']
    info['subscribers'] = int(channel_details['statistics']['subscriberCount'])
    info['views'] = int(channel_details['statistics']['viewCount'])
    info['total_videos'] = int(channel_details['statistics']['videoCount'])
    info['description'] = channel_details['snippet']['description']
    info['country'] = channel_details['snippet']['country']
    return info
...
with st.sidebar:
    selected = option_menu(None, ["Project Description","Process","Questions"], 
                           icons=["house-fill","gear-fill","archive-fill"],
                           default_index=0,
                           orientation="horizontal",
                           styles={"nav-link": {"font-size": "30px", "text-align": "centre", "margin": "0px", "--ahover-color": "#0000FF"},
                                   "icon": {"font-size": "30px"},
...
if selected == "Project Description":
    ...
    col1.markdown("## :blue[Tech Used] : Python,MongoDB, Youtube Data API, MySql, Streamlit")
    col2.markdown("## :blue[Description] : Retrieving the Youtube channels data from the Google API, storing it in a MongoDB as data lake, migrating and transforming data into a SQL database,then querying the data and displaying it in the Streamlit app.")
...
if selected == "Process":
    tab1,tab2 = st.tabs(["$\huge Extract $", "$\huge Load $"])
    ...
    with tab2:     
        ...
        def insert_into_channels():
                ...
                for i in collections.find({"channel_name" : user_inp}):
                    ...
                    values = ", ".join(f"'{value}'" for value in i.values())
                
                    query = f"INSERT INTO channel_details VALUES ({values})"
        ...
        def insert_into_videos():
            ...
            for i in collections.find({"Channel_name" : user_inp}):
                    ...
                    i['Title'] = escaped_title
                    i.pop('Tags',None)
                    
                    # i.pop('Description', None)
                    title = i.get('Description', '')
                    escaped_title = title.replace("'", "''")
                    i['Description'] = escaped_title
                    

                    values = ", ".join(f"'{value}'" for value in i.values())
                    print(values)
                    query = f"INSERT INTO video_details()  VALUES ({values})"
                    mycursor.execute(query)
                    mydb.commit()
        ...
        def insert_into_comments():
            ...
            for vid in collections1.find({"Channel_name" : user_inp}):
                for i in collections2.find({'Video_id': vid['Video_id']}):
                    values = ", ".join(f"'{value}'" for value in i.values())
                    query = f"INSERT INTO comment_details  VALUES ({values})"
                    mycursor.execute(query)
                    mydb.commit()
